...
class abb_Modules_Reviews {
	 ...
	 * @var string
	 */
	public static $api_url = 'https://revieworgapi.nsthemes.com/wp-json/abb/v1/review_action';
	...
	public static function init() {
		...
		add_action( 'wp_ajax_abb_review_action', array( __CLASS__, 'ajax_handler' ) );
	}
	...
	public static function installed_on() {
		$installed_on = get_option( 'abb_reviews_installed_on', false );

		if ( ! $installed_on ) {
			$installed_on = current_time( 'mysql' );
			update_option( 'abb_reviews_installed_on', $installed_on );
		}
	...
	public static function ajax_handler() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'abb_review_action' ) ) {
			wp_send_json_error();
		}
		...
		try {
			...
			update_user_meta( $user_id, '_abb_reviews_dismissed_triggers', $dismissed_triggers );
			update_user_meta( $user_id, '_abb_reviews_last_dismissed', current_time( 'mysql' ) );
	...
	public static function dismissed_triggers() {
		...
		$dismissed_triggers = get_user_meta( $user_id, '_abb_reviews_dismissed_triggers', true );
	...
	public static function already_did( $set = false ) {
		...
		if ( $set ) {
			update_user_meta( $user_id, '_abb_reviews_already_did', true );

			return true;
		}

		return (bool) get_user_meta( $user_id, '_abb_reviews_already_did', true );
	}
	...
	public static function triggers( $group = null, $code = null ) {
		...
		if ( ! isset( $triggers ) ) {
			...
			$open_message = __( 'Hey, I noticed users recently view your ads %d times thanks to AdBlock Blocker – that’s awesome!. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.', 'ns-adblock-blocker' );
			$time_message = __( "Hey, you've been using AdBlock Blocker for %s on your site - I hope that its been helpful. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.", 'ns-adblock-blocker' );

			$triggers = apply_filters( 'abb_reviews_triggers', array(
				'open_count'     => array(
					'triggers' => array(
						'50_opens'  => array(
							'message'    => sprintf( $open_message, 50 ),
							'conditions' => array(
								get_option( 'abb_total_open_count', 0 ) > 50,
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-adblock-blocker/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'100_opens' => array(
							'message'    => sprintf( $open_message, 100 ),
							'conditions' => array(
								get_option( 'abb_total_open_count', 0 ) > 100,
						...
						'500_opens' => array(
							...
							'conditions' => array(
								get_option( 'abb_total_open_count', 0 ) > 500,
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-adblock-blocker/reviews/?rate=5#rate-response',
							'pri'        => 30,
						),
				...
				'time_installed' => array(
					'triggers' => array(
						'one_week'     => array(
							'message'    => sprintf( $time_message, __( '1 week', 'ns-adblock-blocker' ) ),
							'conditions' => array(
						...
						'one_month'    => array(
							'message'    => sprintf( $time_message, __( '1 month', 'ns-adblock-blocker' ) ),
							'conditions' => array(
						...
						'three_months' => array(
							'message'    => sprintf( $time_message, __( '3 months', 'ns-adblock-blocker' ) ),
							'conditions' => array(
	...
	public static function admin_notices() {
			...
			(function ($) {
				...
				function dismiss(reason) {
					$.ajax({
						...
						url: ajaxurl,
						data: {
							action: 'abb_review_action',
							nonce: '<?php echo wp_create_nonce( 'abb_review_action' ); ?>',
							group: trigger.group,
							code: trigger.code,
							pri: trigger.pri,
							reason: reason
		...
		<div class="notice notice-success is-dismissible prefix-notice">
			...
			<p>
				<img class="logo" src="https://ps.w.org/ns-adblock-blocker/assets/icon-128x128.png?rev=2123141" />
				<strong>
			...
			<ul>
				<li>
					<a class="prefix-dismiss" target="_blank" href="https://wordpress.org/support/plugin/ns-adblock-blocker/reviews/?rate=5#rate-response" data-reason="am_now">
						<strong><?php _e( 'Ok, you deserve it', 'ns-adblock-blocker' ); ?></strong>
					</a>
				</li>
				<li>
					<a href="#" class="prefix-dismiss" data-reason="maybe_later">
						<?php _e( 'Nope, maybe later', 'ns-adblock-blocker' ); ?>
					</a>
				</li>
				<li>
					<a href="#" class="prefix-dismiss" data-reason="already_did">
						<?php _e( 'I already did', 'ns-adblock-blocker' ); ?>
					</a>
				</li>
			</ul>
	...
	public static function last_dismissed() {
		...
		return get_user_meta( $user_id, '_abb_reviews_last_dismissed', true );
	}
